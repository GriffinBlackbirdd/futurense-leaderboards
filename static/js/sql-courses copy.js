...
const SQLCourses = (function() {
    ...
    async function _downloadResources(sessionId) {
        try {
            ...
            const response = await fetch(`/api/resources/${sessionId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
            ...
            _elements.downloadBtn.innerHTML = '<i class="fas fa-download"></i> Download Resources';
        }
    }
        ...
        try {
            ...
            loadingTask.onProgress = function(progress) {
                if (progress.total) {
                    ...
                    _elements.loadingEl.textContent = `Loading PDF... ${Math.round(percent)}%`;
                }
            };
    ...
    async function _createSessionCards() {
        ...
        try {
            ...
            _elements.sessionsGrid.innerHTML = '<div class="loading">Loading courses...</div>';
            ...
            if (!sessions || sessions.length === 0) {
                ...
                _elements.sessionsGrid.innerHTML = '<div class="error">No courses available. Please try again later.</div>';
                return;
            }
            ...
            _elements.sessionsGrid.innerHTML = '<div class="error">Error loading courses. Please try again later.</div>';
        }
    }
    ...
    async function init() {
        ...
        try {
            ...
            if (_elements.sessionsGrid) {
                _elements.sessionsGrid.innerHTML = '<div class="error">Failed to initialize courses. Please refresh the page.</div>';
            }
        }
    }
