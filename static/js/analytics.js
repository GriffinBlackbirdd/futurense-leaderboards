...
function calculateKPIs(data) {
  const totalDSA = data.reduce(
    (sum, row) =>
      sum + (row.dsa_easy || 0) + (row.dsa_medium || 0) + (row.dsa_hard || 0),
    0
  );
  ...
  const totalContent = data.reduce(
    (sum, row) =>
      sum + (row.youtube_checked ? 1 : 0) + (row.linkedin_checked ? 1 : 0),
    0
  );

  const totalAptitude = data.reduce(
    (sum, row) =>
      sum + (row.aptitude_attempted ? row.aptitude_questions || 0 : 0),
    0
  );
...
function createDSALineChart(weeklyData) {
  ...
  const layout = {
    ...
    yaxis: {
      ...
      range: [
        0,
        Math.max(...weeklyData.map((d) => Math.max(d.easy, d.medium, d.hard))) +
          1 || 5,
      ],
    },
  };
  ...
  Plotly.newPlot(
    "dsa-line-chart",
    [traceEasy, traceMedium, traceHard],
    layout,
    {
      responsive: true,
...
function processSQLData(data, weekNumber) {
  ...
  const total = weekData.reduce(
    (acc, day) => acc + (parseInt(day.sql_questions) || 0),
    0
  );
...
function processAptitudeData(data, weekNumber) {
  ...
  const total = weekData.reduce(
    (acc, day) => acc + (parseInt(day.aptitude_questions) || 0),
    0
  );
...
function initializeWeekSelectors(data) {
  document
    .getElementById("dsa-week-selector")
  ...
  document
    .getElementById("sql-week-selector")
    .addEventListener("change", (e) => {
  ...
  document
    .getElementById("aptitude-week-selector")
    .addEventListener("change", (e) => {
...
async function fetchAttendanceData() {
  try {
    ...
    if (data.success && data.attendance) {
      const attendanceStats = calculateAttendanceStats(data.attendance);
...
function calculateAttendanceStats(attendanceRecords) {
  const totalSessions = attendanceRecords.length * 2; // Each day has 2 sessions
  let presentCount = 0;

  attendanceRecords.forEach((record) => {
    if (record.morning_session === "present") presentCount++;
    if (record.afternoon_session === "present") presentCount++;
  });
...
function updateAttendanceChart(stats) {
  ...
  document.getElementById("attended-classes").textContent =
    stats.attendedClasses;
  document.getElementById("missed-classes").textContent = stats.missedClasses;
